...
function obtenerHorariosPorGrupoIncompletos($pdo)
{
    $sql = "SELECT 
                g.group_id,
                g.group_name,
            ...
            JOIN shifts sh ON g.turn_id = sh.shift_id
            LEFT JOIN classrooms r ON sa.classroom_id = r.classroom_id
...
$horarios_por_grupo = obtenerHorariosPorGrupoIncompletos($pdo);

if (empty($horarios_por_grupo)) {
    exit("No se encontraron grupos con materias sin profesor.");
}
...
$zip = new ZipArchive();
$zip_file = __DIR__ . '/Horarios_Por_Grupo_Incompletos.zip';
...
foreach ($horarios_por_grupo as $group_name => $horarios) {
    ...
    $excel_filename = "Horario_Incompleto_" . $safe_group_name . ".xlsx";
